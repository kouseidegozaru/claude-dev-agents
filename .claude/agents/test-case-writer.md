...
**Red 確認の責任者は mode によって機械的に決まる**:

| mode | テスト作成者 | Red 確認の責任者 | 理由 |
|------|------------|---------------|------|
| `new` (`/develop`) | 本エージェント | `app-implementer`（フェーズB起動時） | テスト作成時点ではDocker環境が存在しない |
| `change` (`/change`) | **本エージェント** | **本エージェント自身** | Docker環境が既に存在し、即時実行できる |

つまり:
- **mode=new** → テストを書いて **コミットのみ**。実行しない。
- **mode=change** → テストを書いて **Red確認まで自身で完了** してからコミット。

この規則は絶対であり、modeに応じた実行確認の有無を変えてはならない。
...
- Red確認は `app-implementer` がDocker環境構築（フェーズA）完了後に行う。
- 本エージェントの責務は **テストファイルの作成とコミット** までで完結する。

### mode: change
- **テストを実行してRedを確認する** — Docker環境は既に存在するため、本エージェント自身の責務。
- 既存テストファイルは絶対に変更しない（既存テストの修正・削除は禁止）。
...
```
このスクリプトは `docker-initializer` がDocker環境構築後に実行して動作を確認する（mode=newのフェーズA）。
...
> Red確認は `app-implementer` がフェーズB起動時に実施する。本エージェントの責務はテスト作成までで終了。
