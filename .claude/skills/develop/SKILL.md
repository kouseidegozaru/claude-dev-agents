...
## Step 2: 不足情報の一括質問（3択 + 自由回答）

分析結果をもとに、**一度だけ**まとめて質問します。後から追加質問は行いません。

**重要: 単に「教えてください」と聞かない**。各項目について、命令の文脈・一般的なベストプラクティス・採用実績を踏まえた **3つの選択肢（うち1つを推奨）** を必ず提示し、加えて **自由回答** も受け付けます。ユーザーが最小コストで判断できる形にします。

各質問は以下の構成を必ず守ること:

- **質問内容** — 何を決めたいか（1行）
- **選択肢 A / B / C** — 3つの妥当な候補。`★` を付けたものが Claude の推奨
- **その他** — 自由回答用。ユーザーが独自の答えを書ける枠

フォーマット:
```
以下の情報を教えてください。各項目は「A / B / C / その他」から選んでください。
`★` が付いた選択肢が推奨です。「すべて推奨で OK」「1番はB, 2番は推奨」のような短い回答も受け付けます。
...
1. **言語・フレームワーク**
   - A. Node.js + Express
   - B. Python 3.11 + FastAPI ★推奨（軽量・型ヒント対応・非同期対応で小規模Web APIに最適）
   - C. Go + Gin
   - その他: 自由記述

2. **データ永続化**
   - A. SQLite（ファイルベース） ★推奨（単一ユーザー想定なら最小構成で十分）
   - B. PostgreSQL（Docker サービス追加）
   - C. インメモリ（永続化なし）
   - その他: 自由記述
```
...
- **A / B / C は実際に妥当な候補** にする（極端な選択肢で水増ししない）
- **★推奨の根拠を1〜2文で添える** — 「なぜ推奨か」が分かるように
- **保守的な選択を推奨に** — 採用実績が多く Docker フレンドリーなものを優先
- **命令の規模に合わせる** — 「個人用」と「業務用」で推奨が変わる
- **独断専行しない** — ユーザーが「その他」や別選択肢を選んだら必ず従う

ユーザーの回答（「全部推奨でOK」「1はC, 2は その他: MongoDB」など）を受け取ったら Step 3 に進みます。
