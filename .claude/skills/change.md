...
## Step 4: GitHub Issue作成（サブエージェントに委譲）

`issue-creator` サブエージェントに以下を渡します:
- 変更リクエストと回答のペア一覧
- 変更対象ファイル一覧（`code-analyzer` の結果）
- ラベルは `enhancement` を使うよう指定
- 作業ディレクトリパスとリポジトリ情報
...
- 既存テストファイルの一覧（変更時に参照するため）
- リポジトリ情報

> **注意**: このフェーズでは新しいテストの追加のみ行う。
> 既存テストの削除・修正は禁止（それ自体がリグレッションになる）。
